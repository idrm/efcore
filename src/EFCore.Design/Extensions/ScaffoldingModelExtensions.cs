...
public static class ScaffoldingModelExtensions
{
    ...
    /// <param name="annotationCodeGenerator">The provider's annotation code generator.</param>
    /// <param name="entityNamespace">The entity root namespace.</param>
        ...
        bool useStrings = false)
    {
        ...
        if (foreignKey.IsUnique)
        {
            hasForeignKey.TypeArguments.Add(
                NamespaceNamingHelper.CreateFullyQualifiedName(entityNamespace, foreignKey.DeclaringEntityType.Name)
            );
        }
