...
");
    ...
    foreach (var entityType in Model.GetEntityTypes().Where(e => !e.IsSimpleManyToManyJoinEntityType()))
    {
        ...
        {
            var foreignKeyFluentApiCalls = foreignKey.GetFluentApiCalls(annotationCodeGenerator, Options.ModelNamespace)
                ?.FilterChain(c => !(Options.UseDataAnnotations && c.IsHandledByDataAnnotations));
            ...
            this.Write(this.ToStringHelper.ToStringWithCulture(NamespaceNamingHelper.CreateBaseEntityName(foreignKey.DependentToPrincipal.Name)));
            this.Write(").");
            this.Write(this.ToStringHelper.ToStringWithCulture(foreignKey.IsUnique ? "WithOne" : "WithMany"));
            this.Write("(");
            this.Write(this.ToStringHelper.ToStringWithCulture(foreignKey.PrincipalToDependent != null
                ? $"p => p.{NamespaceNamingHelper.CreateBaseEntityName(foreignKey.PrincipalToDependent.Name)}" : ""));
            this.Write(")");
        ...
        foreach (var skipNavigation in entityType.GetSkipNavigations().Where(n => n.IsLeftNavigation()))
        {
            ...
            var leftFluentApiCalls = left.GetFluentApiCalls(annotationCodeGenerator, Options.ModelNamespace, useStrings: true);
            var right = skipNavigation.Inverse.ForeignKey;
            var rightFluentApiCalls = right.GetFluentApiCalls(annotationCodeGenerator, Options.ModelNamespace, useStrings: true);
            ...
            this.Write(",\r\n                    r => r.HasOne<");
            this.Write(NamespaceNamingHelper.CreateFullyQualifiedName(Options.ModelNamespace, right.PrincipalEntityType.Name));
            this.Write(">().WithMany()");
            this.Write(this.ToStringHelper.ToStringWithCulture(code.Fragment(rightFluentApiCalls, indent: 6)));
            this.Write(",\r\n                    l => l.HasOne<");
            this.Write(NamespaceNamingHelper.CreateFullyQualifiedName(Options.ModelNamespace, left.PrincipalEntityType.Name));
            this.Write(">().WithMany()");
